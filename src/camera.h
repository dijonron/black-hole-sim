...
struct Camera {
  ...
  // Set up the camera
  //   origin: The position of the camera in the scene.
  ...
  Camera(Vec3 origin, Vec3 target, Vec3 up, double vFov, double aspectRatio) {
    double theta      = vFov * M_PI / 180.0;      // Convert FOV to radians
    double halfHeight = std::tan(theta / 2);      // Calculate half the height of the image plane based on FOV
    ...
    Vec3 w = (origin - target).normalized();  // Behind
